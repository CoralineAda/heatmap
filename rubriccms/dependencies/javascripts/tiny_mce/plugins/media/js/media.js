...
new Ajax.Request('/flash_assets/',
  {
    method:'get',
    onSuccess: function(transport){
      flashAssets = eval(transport.responseText) || "no response text";
    },
    onFailure: function(){ alert('Something went wrong...') }
});

new Ajax.Request('/quicktime_assets/',
  {
    method:'get',
    onSuccess: function(transport){
      qtAssets = eval(transport.responseText) || "no response text";
    },
    onFailure: function(){ alert('Something went wrong...') }
});
...
function setFileList(t) {
  switch (t) {
    case 'flash':
      $('src').options.length = 0;
      if (flashAssets[0]) {
        flashAssets.each(function(a,index){
          if (a.filename && a.url) {
            $('src').options[index + 1] = new Option(a.filename, a.url)
          }
        });
      }
      break;
    case 'qt':
      $('src').options.length = 0;
      if (qtAssets[0]) {
        qtAssets.each(function(a,index){
          if (a.filename && a.url) {
            $('src').options[index + 1] = new Option(a.filename, a.url)
          }
        });
...
function generatePreview(c) {
	...
	p.innerHTML = "<!-- x --->" + h + "<br style='clear: both;' />";
}
